...
Debería listar el `Master-Prompt.md` en `Orchestrator/` de la fuente, los doce archivos de reglas por categoría `Rules-*.md` (de `Rules-Contexto.md` a `Rules-Examples.md`), más las cinco reglas transversales `Root-Rules.md`, `Intake-Rules.md`, `Maqueta-Rules.md`, `Deriva-Rules.md` y `Vocabulario-Rules.md` en `Rules/` de la fuente —diecisiete archivos en total—, y tu intake personalizado en `SDD/Intake/` del destino (el único `PRODUCT-INTAKE` del producto). El `PRODUCT-MANIFEST` no lo creás vos: lo deriva el orquestador y lo escribe también en `SDD/Intake/` del destino.
...
Caso típico: el cliente cambia el alcance a mitad del proyecto de código. Por ejemplo, en el caso "API REST de turnos médicos", el cliente decide en el sprint 2 que sí va a haber app móvil nativa en v1 (estaba como Won't Have).
...
| Procedencia del framework | Bloque del `PRODUCT-MANIFEST` que declara bajo qué versión del framework y de qué reglas se generó la documentación del destino. Es lo que hace posible la reconciliación normativa. |
